  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {[...Array(2)].map((_, i) => (
          <Card key={i}><CardContent className="p-5"><Skeleton className="h-80 w-full skeleton-shimmer" /></CardContent></Card>
        ))}
      </div>
...
function HeadToHeadItem({
  ...
  const diffPct = item.competitor > 0
    ...
    >
      ...
      <div className="flex h-2.5 rounded-full overflow-hidden bg-muted">
        <div
          ...
          style={{ width: `${primaryPct}%`, backgroundColor: PRIMARY_COLOR }}
        />
      </div>
      <div className="flex justify-between mt-1.5 text-xs text-muted-foreground">
        <span className="tabular-nums">
          {primary_brand ?? 'Primary'}: {item.primary >= 1000
            ? formatCompact(item.primary)
            : item.primary.toFixed(item.primary < 10 ? 1 : 0)}
        </span>
        <span className="tabular-nums">
          {competitor_brand ?? 'Competitor'}: {item.competitor >= 1000
...
export default function BenchmarkPage() {
  ...
  const { ref: gapRef, isVisible: gapVisible } = useScrollReveal<HTMLDivElement>({ threshold: 0.05, delay: 150 });
  ...
  const { radar, head_to_head, gap_analysis, primary_brand, competitor_brand } = data;
      ...
      <div ref={sectionRef} className={`section-header-reveal ${sectionVisible ? 'is-visible' : ''}`}>
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Comparative analysis — {primary_brand ?? 'Primary Brand'} vs {competitor_brand ?? 'Competitor'}
          </p>
        </div>
      </div>
            ...
            <Card>
              <CardContent className="p-5">
                <div className="relative">
                  {/* Radial gradient backdrop behind radar */}
                  <div
                    className="absolute inset-0 rounded-b-lg pointer-events-none"
                    style={{
                      background: 'radial-gradient(ellipse at 50% 40%, oklch(0.9 0.08 266 / 0.15) 0%, transparent 70%)',
                      zIndex: 0,
                    }}
                  />
                <div className="flex items-center justify-between mb-2">
